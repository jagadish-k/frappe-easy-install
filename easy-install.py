...
def generate_config_files(
    ...
    """Generate .env and docker-compose.yml files without starting containers"""
    ...
    compose_file_name = os.path.join(output_dir, f"{project}-compose.yml")
    env_file_name = f"{project}.env"
    env_file_path = os.path.join(output_dir, env_file_name)
    passwords_file_path = os.path.join(output_dir, f"{project}-passwords.txt")
    ...
    write_to_env(
        ...
        http_port=http_port if not is_https or no_proxy else None,
        custom_image=custom_image,
        custom_tag=custom_tag,
    )

    # Generate base docker-compose file with updated override logic
    try:
        # Updated compose override selection logic
        compose_override = (
            "overrides/compose.https.yaml"
            if is_https and not no_proxy
        ...
        # Get the generated compose content
        ...
        try:
            ...
            for i, line in enumerate(lines):
                if line and not line.startswith(' ') and not line.startswith('\t') and ':' in line:
                    ...
                    if section_name in ['networks', 'volumes', 'configs', 'secrets']:
                        insert_index = i
                        break
            ...
            for i, site in enumerate(sites):
                service_name = f"create-site-{site.replace('.', '-')}"
                ...
                for app in apps:
                    create_site_command.extend(["--install-app", app])
                
                create_site_command.append(site)
                
                command_yaml = "    - " + "\n    - ".join([f'"{cmd}"' for cmd in create_site_command])
                
                if i == 0:
                    additional_services.append("")
                
                additional_services.extend([
                    f"{indent}{service_name}:",
                    f"{indent * 2}extends:",
                    f"{indent * 3}service: backend",
                    f"{indent * 2}deploy:",
                    f"{indent * 3}restart_policy:",
                    f"{indent * 4}condition: \"none\"",
                    f"{indent * 2}command:",
                    command_yaml,
                ])
            ...
            lines[insert_index:insert_index] = additional_services
            compose_content = '\n'.join(lines)
            
        except Exception as e:
            logging.error("Failed to add create-site services", exc_info=True)
            cprint(f"Failed to add create-site services: {e}", level=3)
    ...
    with open(passwords_file_path, "w") as pf:
        pf.writelines(f"ADMINISTRATOR_PASSWORD={admin_pass}\n")
        pf.writelines(f"MARIADB_ROOT_PASSWORD={db_pass}\n")

    cprint(f"✓ Environment file generated: {env_file_path}", level=2)
    cprint(f"✓ Docker Compose file generated: {compose_file_name}", level=2)
    if sites and apps:
        cprint(f"✓ Create-site services added for: {', '.join(sites)}", level=2)
    cprint(f"✓ Passwords file generated: {passwords_file_path}", level=2)
    cprint(f"\nMariaDB root password: {db_pass}", level=3)
    cprint(f"Site administrator password: {admin_pass}", level=3)
    
    if sites and apps:
        cprint(f"\nTo create sites, run:", level=3)
        for site in sites:
            service_name = f"create-site-{site.replace('.', '-')}"
            cprint(f"  docker compose -f {os.path.basename(compose_file_name)} run --rm {service_name}", level=3)
    
    return env_file_path, compose_file_name, passwords_file_path
...
def add_common_parser(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--no-proxy", action="store_true", help="No proxy/reverse proxy")
    parser.add_argument(
        "-v",
        "--version",
...
def add_generate_parser(subparsers: argparse.ArgumentParser):
    parser = subparsers.add_parser("generate", help="Generate .env and docker-compose.yml files without starting containers")
    parser = add_common_parser(parser)
    parser = add_setup_options(parser)
    parser.add_argument(
        "-o",
...
if __name__ == "__main__":
    ...
    if args.subcommand == "build":
        ...
        logging.info("Running Configuration Generation")
        if args.email and "example.com" in args.email:
            cprint("Emails with example.com not acceptable", level=1)
            sys.exit(1)
        
        if len(args.sites) == 0:
            args.sites = ["site1.localhost"]
